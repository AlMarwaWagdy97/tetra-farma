    ...
    <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-12">
                            ...
                            <div id="collapseLang{{ $key }}" class="accordion-collapse collapse show" data-bs-parent="#accordionLang{{ $key }}">
                                <div class="accordion-body">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="{{ $locale }}[title]" class="form-control" value="{{ old($locale . '.title', optional($blog->translate($locale))->title) }}">
                                    </div>

                                

                                    <div class="mb-3">
                                        <label class="form-label">@lang('blogs.description')</label>
                                        <textarea id="description{{ $key }}" name="{{ $locale }}[description]" class="form-control" rows="6">{{ old($locale . '.description', optional($blog->translate($locale))->description) }}</textarea>
                                    </div>
                ...
                <div class="mb-3">
                    <label class="form-label">{{ __('admin.image') }}</label><br>
                    @if ($blog->image)
