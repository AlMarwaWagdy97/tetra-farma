{{-- Reviews --}}
<div class="Review my-4 p-5 text-center">
    <div class="container">
        <h5>{{ app()->getLocale() == 'ar' ? 'آراء عملائنا' : 'Our Customers Reviews' }}</h5>
        <h3>{{ app()->getLocale() == 'ar' ? 'آراء أكثر من 100 عميل' : 'Reviews from over 100 customers' }}</h3>
        <div class="swiper ReviewSlider">
            <div class="swiper-wrapper mt-3">
                @forelse ($reviews as $review)
                    <div class="swiper-slide">
                        <div class="Reviewbox mx-auto p-3 rounded" 
                            ...
                            <div class="rate text-center mt-2">
                                @php
                                    ...
                                    $filledStars = floor($review->rate);
                                    $hasHalfStar = $review->rate - $filledStars >= 0.5;
                                @endphp
                                @for ($i = 1; $i <= 5; $i++)
                                    <i class="fa-solid fa-star {{ $i <= $filledStars ? 'text-warning' : ($i == $filledStars + 1 && $hasHalfStar ? 'text-warning half' : 'text-secondary') }}"></i>
                                @endfor
                                <span class="text-muted">({{ number_format($review->rate, 1) }})</span>
                            </div>
                        </div>
                    </div>
                @empty
